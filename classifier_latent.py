...
class CLASSIFIER:
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                mus,stds,dis_out,pred,encoder_out = self.map(inputv)
    ...
    def val_gzsl(self, test_X, test_label, target_classes):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                mus,stds,dis_out,pred,encoder_out = self.map(test_X[start:end].cuda())
                output = self.model(mus)
            else:

                mus,stds,dis_out,pred,encoder_out = self.map(test_X[start:end])
                output = self.model(mus)
